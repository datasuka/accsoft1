...
def buat_voucher(df, no_voucher, settings):
    ...
    for _, row in data.iterrows():
        ...
        # --- hitung tinggi baris (max line per kolom) ---
        line_counts = []
        ...
        # --- gambar kotak tiap kolom dengan tinggi sama ---
        x_start = pdf.get_x()
        y_start = pdf.get_y()
...
if file:
    ...
    if mode == "Single Voucher":
        ...
        if st.button("Cetak Semua Voucher Bulan Ini"):
            ...
            st.download_button("⬇️ Download ZIP", data=buffer_zip, file_name=f"voucher_{nama_bulan}.zip", mime="application/zip")
