...
def buat_voucher(df, no_voucher, settings):
    ...
    if settings.get("logo"):
        pdf.image(settings["logo"], 15, 8, settings.get("logo_size", 25))

    pdf.set_xy(45, 10)
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 6, settings.get("perusahaan", ""), ln=1, align="L")
    pdf.set_x(45)
    pdf.set_font("Arial", "", 10)
    pdf.multi_cell(0, 5, settings.get("alamat", ""), align="L")
    pdf.ln(8)
    ...
    for _, row in data.iterrows():
        ...
        # hitung tinggi baris sinkron
        line_counts = [len(pdf.multi_cell(w, 6, val, split_only=True)) for val, w in zip(values, col_widths)]
