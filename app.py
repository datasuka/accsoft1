...
def buat_voucher(df, no_voucher, settings):
    ...
    for _, row in data.iterrows():
        ...
        # Sinkronisasi Nama Akun & Deskripsi
...
if file:
    ...
    if pilihan == "Single Voucher":
        ...
        if st.button("Download Voucher Jurnal"):
            ...
            st.download_button("⬇️ Download PDF", data=pdf_file, file_name=f"{no_voucher}.pdf", mime="application/pdf")

    else:
        if st.button("Download Semua Voucher"):
            buffer = BytesIO()
            with zipfile.ZipFile(buffer, "w") as zf:
                for no_voucher in df["Nomor Voucher Jurnal"].unique():
                    pdf_file = buat_voucher(df, no_voucher, settings)
                    zf.writestr(f"{no_voucher}.pdf", pdf_file)
            buffer.seek(0)
            st.download_button("⬇️ Download ZIP", data=buffer, file_name="voucher_jurnal_all.zip", mime="application/zip")
